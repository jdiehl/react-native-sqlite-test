...
export async function runTest() {
  ...
  db.executeSql('PRAGMA journal_mode = MEMORY;'); // or OFF

  const r: any = {};
  r.c2 = await measureExecutionTime(
    async () => await db.executeSql(SQL_CREATE),
  );
  ...
  r.s2 = await measureExecutionTime(async () => {
    for (let i = 0; i < C_SELECT; i++) {
      await db.executeSql(SQL_SELECT + ' WHERE ID >= ?', [i]);
    }
  });
  r.d2 = await measureExecutionTime(async () => await db.executeSql(SQL_DROP));
